...
private[spark] class DAGScheduler(
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int): Unit = {
    ...
    val taskIdToLocations: Map[Int, Seq[TaskLocation]] = try {
      stage match {
        case s: ShuffleMapStage =>
          partitionsToCompute.map { id => (id, getPreferredLocs(stage.rdd, id, stage.id))}.toMap
        case s: ResultStage =>
          partitionsToCompute.map { id =>
            val p = s.partitions(id)
            (id, getPreferredLocs(stage.rdd, p, stage.id))
          }.toMap
      }
    } catch {
      case NonFatal(e) =>
    ...
    logWarning(s"==================current stage is ${stage.id}\n")
    logWarning(s"==================task ids are ${taskIdToLocations.keys.toSeq}\n")
    taskIdToLocations.foreach(t =>
      logWarning(s"=============task locations are $t\n"))
   ...
   */
  private[spark]
  def getPreferredLocs(rdd: RDD[_], partition: Int, stageId: Int): Seq[TaskLocation] = {
    var locs = getPreferredLocsInternal(rdd, partition, new HashSet)
    logWarning(s"the locations of rdd-${rdd.name} are $locs")
    if (locs.nonEmpty&&List(9, 10, 11, 12, 14, 16, 17, 19, 21, 26).contains(stageId)) {
      locs = locs.map(_ => TaskLocation("slave2"))
    }
    locs
  }
  ...
  private def getPreferredLocsInternal(
    ...
    val rddName = rdd.name
    var locs: Seq[TaskLocation] = Nil
    try {
      if (!visited.add((rdd, partition))) {
        // Nil has already been returned for previously visited partitions.
        logWarning(s"===========partition $partition visited")
        return locs
      }
      // If the partition is cached, return the cache locations
      val cached = getCacheLocs(rdd)(partition)
      if (cached.nonEmpty) {
        logWarning(s"===========partition $partition cached")
        locs = cached
        return locs
      }
      ...
      if (rddPrefs.nonEmpty) {
        logWarning(s"===========partition $partition has preferred location")
        locs = rddPrefs.map(TaskLocation(_))
        return locs
      }
      ...
      rdd.dependencies.foreach {
        case n: NarrowDependency[_] =>
          logWarning(s"===========partition $partition has narrowDependency")
          for (inPart <- n.getParents(partition)) {
            locs = getPreferredLocsInternal(n.rdd, inPart, visited)
            if (locs != Nil) {
              return locs
            }
          }
      ...
      logWarning(s"finished find ${rddName}'s partition location of partition $partition locs" +
        s" is $locs")
    }
  }
